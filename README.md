...
| `workflow-automations` | `site-capture`, `browser`, `create-agent-harness`, `create-streamlit-app`, `verify-streamlit-app` | Record scripted website walkthroughs as video or GIF, read and act on websites in your logged-in Chrome, build skill-driven Python agents against OpenAI-compatible endpoints, and scaffold local Streamlit data apps verified through a headless browser. |

...
## Reference footage and voice selection

For a visual-only recording, create a silent low-resolution proxy before extracting frames. Verify that it has no audio stream; preserve the original. Inspect a sparse contact sheet first, then a short denser crop where mouth motion is visible. Discard scene cuts and interface overlays as evidence. Silent footage can reveal mouth shapes, asymmetry and head/hand phrasing, but cannot validate phoneme timing. Small crops also cannot establish detailed lip contacts.

Keep voice generation separate from alignment and rig deformation. A local Kokoro audition produced usable files and one output was passed through Rhubarb; no stock voice was established as a specific character match. An RVC character model is a voice-conversion stage after a spoken performance, not a text-to-speech preset. Qwen3-TTS VoiceDesign 1.7B 8-bit with MLX-Audio was subsequently tested locally on Apple Silicon: three English character prompts generated 7–9-second auditions in approximately 4–9 seconds each after initial setup. These are observed generation timings on one machine, not a quality benchmark. Save the voice description, model, seed and output together; prompt intent does not prove the resulting voice sounds like the requested character. Keep the environment isolated from existing TTS installations. Align the final audio after any voice conversion or time/pitch processing, and resize the facial Action and sound strip together when the new utterance exceeds the old slot. Never reuse cue timestamps merely because the transcript is unchanged.

For a looping speech preview, test silent lead-in and tail frames as well as cue centers. Facial NLA influence fades can blend a correct closed-mouth Action against an open-smile base, causing visible speech-like motion while the audio is silent. Hold the closed facial pose at full influence across the loop's silent handles, or provide a matching closed base. Verify half-frame samples on both boundaries. When translating later NLA clips to fit longer dialogue, move both start and end explicitly; check duration and compare saved-file poses at offset frames rather than assuming assigning `frame_start` shifts the strip intact.
